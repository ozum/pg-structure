...
function getForeignKeyColumns(callback) {
    ...
    return helper.getCollection(columns, callback, { objectKey: null });
}
...
function relatedTableGetter(type, nameKey) {
    ...
    return function getRelatedTables(callback) {
        ...
        return helper.getCollection(tables, callback, {objectKey: null, nameKey: nameKey});
    };
}
...
/**
 * Executes callback for each realtion or returns all relations of table.
 * @private
 * @param {orderedRelationCallback} [callback]                      - Calbback to execute for each relation.
 * @returns {Array.<Column>|Object.<String, Object>|null|undefined} - List of relations.
 */
function getAllRelations(callback) {
    var result = this.o2mRelations.concat(this.m2oRelations, this.m2mRelations);
    return helper.getCollection(result, callback, {objectKey: null});
}

/**
 * @name Table#relations
 * @type {Array.<O2MRelation|M2ORelation|M2MRelation>|null}
 * @readonly
...
function createIndexGetter(filter) {
     ...
     * @private
     * @param {orderedIndexCallback}   [callback]  - Callback function.
     * @returns {Array.<Index>|null}                - Array of unique {@link Index indexes} column is part of.
     */
    return function indexGetter(callback) {
        filter = filter || {};
        filter.tableFullCatalogName = this.fullCatalogName;

        let _ = internal.get(this);
        let indexes     = _.registry.getCollection('index').findObjects(filter)
            .sort((a, b) => a.name.toLowerCase() <= b.name.toLowerCase() ? -1 : 1);

        return helper.getCollection(indexes, callback);
    };
}

/**
 * @name Table#indexes
 * @type {Array.<Index>|null}
 * @readonly
 ...
 * @returns {Array.<Column>|undefined|null}     - All {@link Column columns} of the table.
 * @example
 ...
 * );
 */
Object.defineProperty(Table.prototype, 'getColumns', { value: helper.createObjectsGetter('column', {parent: '$this.fullCatalogName'}), enumerable: false });
 ...
 * @returns {Array.<Constraint>|undefined|null}     - All {@link Constraint constraints} of the table.
 ...
 * );
 */
Object.defineProperty(Table.prototype, 'getConstraints', { value: helper.createObjectsGetter('constraint', {parent: '$this.fullCatalogName'}), enumerable: false });
 ...
 * ({@link Constraint}, index, collection). If no callback is provided, returns an array of all foreign key {@link Constraint constraints}.
 * @method Table#getForeignKeyConstraints
 * @param {orderedConstraintCallback} [callback]    - Callback to be executed for each constraint.
 * @returns {Array.<Constraint>|undefined|null}     - All foreign key {@link Constraint constraints} of the table.
 ...
 * );
 */
Object.defineProperty(Table.prototype, 'getForeignKeyConstraints', { value: helper.createObjectsGetter('constraint', {parent: '$this.fullCatalogName', constraintType: 'FOREIGN KEY'}), enumerable: false });
 ...
 * ({@link Column}, index, collection). If no callback is provided, returns an array of all foreign key {@link Column columns}.
 * @method Table#getForeignKeyColumns
 * @param {orderedColumnCallback} [callback]    - Callback to be executed for each column.
 * @returns {Array.<Column>|undefined|null}     - All foreign key {@link Column columns} of the table.
 * @example
 ...
 * ({@link Column}, index, collection). If no callback is provided, returns an array of all primary key {@link Column columns}.
 * @method Table#getPrimaryKeyColumns
 * @param {orderedColumnCallback} [callback]    - Callback to be executed for each column.
 * @returns {Array.<Column>|undefined|null}     - All primary key {@link Column columns} of the table.
 * @example
 ...
 * ({@link Constraint}, index, collection). If no callback is provided, returns single element array of primary key {@link Constraint constraint}.
 ...
 * @returns {Array.<Constraint>|undefined|null}     - Primary key {@link Constraint constraint} of the table.
 * @example
 ...
 * );
 */
Object.defineProperty(Table.prototype, 'getPrimaryKeyConstraint', { value: helper.createObjectsGetter('constraint', {parent: '$this.fullCatalogName', constraintType: 'PRIMARY KEY'}), enumerable: false });
 ...
 * @returns {Array.<Column>|undefined|null} - All {@link Table tables} which this table has relationship of type `one to many`.
 */
Object.defineProperty(Table.prototype, 'getHasManyTables', { value: relatedTableGetter('hasMany'), enumerable: false });
 ...
 * @returns {Array.<Column>|undefined|null} - All {@link Table tables} which this table has relationship of type `one to many`.
 */
Object.defineProperty(Table.prototype, 'getBelongsToTables', { value: relatedTableGetter('belongsTo'), enumerable: false });
 ...
 * @param {orderedTableCallback} [callback] - Callback to be executed for each {@link Table table}.
 * @returns {Array.<Column>|undefined|null} - All {@link Table tables} which this table has relationship of type `many to many`.
 */
Object.defineProperty(Table.prototype, 'getBelongsToManyTables', { value: relatedTableGetter('belongsToMany'), enumerable: false });
 ...
 * @returns {Array.<M2MRelation>|undefined|null}    - {@link M2MRelation M2M Relations} of the table.
 */
Object.defineProperty(Table.prototype, 'getM2MRelations', { value: helper.createObjectsGetter(
    'referentialConstraint', {leftTableFullCatalogName: '$this.fullCatalogName', rightTableFullCatalogName: { $ne: null } }, { objectKey: 'm2mObject' }
), enumerable: false });
 ...
 * @returns {Array.<O2MRelation>|undefined|null}    - {@link O2MRelation O2M Relations} of the table.
 */
Object.defineProperty(Table.prototype, 'getO2MRelations', { value: helper.createObjectsGetter(
    'referentialConstraint', {leftTableFullCatalogName: '$this.fullCatalogName'}, { objectKey: 'o2mObject' }
), enumerable: false });
 ...
 * @returns {Array.<M2ORelation>|undefined|null}    - {@link M2ORelation M2O Relations} of the table.
 */
Object.defineProperty(Table.prototype, 'getM2ORelations', { value: helper.createObjectsGetter(
    'referentialConstraint', {joinTableFullCatalogName: '$this.fullCatalogName'}, { objectKey: 'm2oObject' }
), enumerable: false });

/**
 * List of all {@link Relation relationships} of the table.
 * @method Table#getRelations
 * @returns {Array.<O2MRelation|M2ORelation|M2MRelation>|null}
 */
Object.defineProperty(Table.prototype, 'getRelations', { value: getAllRelations, enumerable: true });

/**
 * Executes callback for every {@link Index} or returns list of {@link Index indexes}, which this table has.
 * Results are ordered by index name.
 * @method Table#getIndexes
 * @param {orderedIndexCallback} [callback] - Callback to be executed for each {@link Index}.
 * @returns {Array.<Index>|undefined|null}  - {@link Index indexes} this table has.
 */
Object.defineProperty(Table.prototype, 'getIndexes', { value: createIndexGetter(), enumerable: false });

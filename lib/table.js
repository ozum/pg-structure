...
function getM2ORelations() {
    return Array.from(this.foreignKeyConstraints.values()).map(c => new M2O({
        sourceTable: c.table,
...
function getO2MRelations() {
    return Array.from(this.foreignKeyConstraintsToThis.values()).map(c => new O2M({
...
function *throughConstraints(onlyPk) {
    for (let fkToThis of this.foreignKeyConstraintsToThis.values()) {
        if (onlyPk && !Array.from(fkToThis.columns.values()).every(c => c.isPrimaryKey)) { continue; }         // Skip if not all columns are PK

        for (let fkToOther of fkToThis.table.foreignKeyConstraints.values()) {
            if (fkToThis === fkToOther) { continue; }                                                   // Skip if it is itself.
            if (onlyPk && !Array.from(fkToOther.columns.values()).every(c => c.isPrimaryKey)) { continue; }    // Skip if not all columns are PK
 ...
 * @type {Object}
 * @readonly
 * @description JS Object extracted from table description. Object is expected as JSON data between `[JSON]` and `[/JSON]`

...
function createIndexGetter(filter) {
    ...
    return function indexGetter(callback) {
        filter = filter || {};
        filter.columnFullCatalogName = this.fullCatalogName;

        let _ = internal.get(this);
        let indexes     = _.registry.getCollection('indexColumn').findObjects(filter)
            .sort((a, b) => a.indexName.toLowerCase() <= b.indexName.toLowerCase() ? -1 : 1)
            .map((value) => _.registry.getCollection('index').by('fullCatalogName', value.indexFullCatalogName).object);

        return helper.getCollection(indexes, callback, {objectKey:null});
    };
}
 ...
 * @readonly
 * @description List of unique {@link Index indexes}, which column is part of. Results are ordered by index name. Excludes primary key indexes. PostgreSQL already creates a unique index for unique
 ...
 */
Object.defineProperty(Column.prototype, 'uniqueIndexesNoPK', { get: createIndexGetter({ isUnique: true, isPrimaryKey: false }), enumerable: true });

/**
 * @name Column#uniqueIndexes
 * @type {Array.<Index>|null}
 * @readonly
 * @description List of unique {@link Index indexes}, which column is part of. Results are ordered by index name. PostgreSQL already creates a unique index for unique
 * {@link Constraint constraints}. So there is no need to look for unique constraints which will result duplicates.
 * @see {@link Column#uniqueIndexesNoPK uniqueIndexesNoPK} for unique indexes excluding primary key indexes.
 */
Object.defineProperty(Column.prototype, 'uniqueIndexes', { get: createIndexGetter({ isUnique: true }), enumerable: true });

/**
 * @name Column#indexes
 * @type {Array.<Index>|null}
 * @readonly
 * @description List of {@link Index indexes}, which column is part of. Results are ordered by index name.
 */
Object.defineProperty(Column.prototype, 'indexes', { get: createIndexGetter(), enumerable: true });

// METHODS

/**
 * Executes callback for every unique {@link Index} or returns list of unique {@link Index indexes}, which column is part of.
 * Results are ordered by index name. Excludes primary key indexes. PostgreSQL already creates a unique index for unique
 * {@link Constraint constraints}. So there is no need to look for unique constraints which will result duplicates.
 * @method Column#getUniqueIndexesNoPK
 * @param {orderedIndexCallback} [callback] - Callback to be executed for each unique {@link Index}.
 * @returns {Array.<Index>|undefined|null}  - Unique {@link Index indexes} this column is a part of.
 * @see {@link Column#getUniqueIndexes getUniqueIndexes} for all unique indexes including primary key indexes.
 */
Object.defineProperty(Column.prototype, 'getUniqueIndexesNoPK', { value: createIndexGetter({ isUnique: true, isPrimaryKey: false }), enumerable: false });

/**
 * Executes callback for every unique {@link Index} or returns list of unique {@link Index indexes}, which column is part of.
 * Results are ordered by index name. PostgreSQL already creates a unique index for unique
 * {@link Constraint constraints}. So there is no need to look for unique constraints which will result duplicates.
 * @method Column#getUniqueIndexes
 * @param {orderedIndexCallback} [callback] - Callback to be executed for each unique {@link Index}.
 * @returns {Array.<Index>|undefined|null}  - Unique {@link Index indexes} this column is a part of.
 * @see {@link Column#getUniqueIndexesNoPK getUniqueIndexesNoPK} for all unique indexes excluding primary key indexes.
 */
Object.defineProperty(Column.prototype, 'getUniqueIndexes', { value: createIndexGetter({ isUnique: true }), enumerable: false });

/**
 * Executes callback for every {@link Index} or returns list of {@link Index indexes}, which column is part of.
 * Results are ordered by index name.
 * @method Column#getIndexes
 * @param {orderedIndexCallback} [callback] - Callback to be executed for each {@link Index}.
 * @returns {Array.<Index>|undefined|null}  - {@link Index indexes} this column is a part of.
 */
Object.defineProperty(Column.prototype, 'getIndexes', { value: createIndexGetter(), enumerable: false });

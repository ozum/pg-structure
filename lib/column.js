...
let attributes  = [
    ...
    'defaultWithTypeCast', ['description', 'comment'], ['enumValues', 'enumLabels'],
...
let parameters  = [
    'allowNull', 'arrayDimension', 'arrayType',
    'defaultWithTypeCast', 'description', 'domainName', 'domainSchema', 'isAutoIncrement',
...
function getIndexes() {
    return [...this.table.indexes.entries()].filter(e => e[1].columns.has(this.name));
}

function getForeignKeyConstraints() {
    return [...this.table.foreignKeyConstraints.entries()].filter(e => e[1].columns.has(this.name));
}
...
function getUniqueIndexes() {
    return [...this.indexes.entries()].filter(e => e[1].isUnique);
}

function getUniqueIndexesNoPk() {
    return [...this.uniqueIndexes.entries()].filter(e => !e[1].isPrimaryKey);
}

function getReferencedColumns() {
    let columns = new ASet;
    for (let fkc of [...this.foreignKeyConstraints.values()]) {

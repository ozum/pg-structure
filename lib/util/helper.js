...
const reJSON = /\s*?\[pg-structure]((.|[\s])+?)\[\/pg-structure]\s*?/im; // [JSON]{"key": "value"}[/PGEN]
...
module.exports.createAccessors = function createAccessors(weakMap, Constructor, attributes) {
    for (let attributeList of attributes) {
        ...
        for (let attribute of attributeList) {
            Object.defineProperty(Constructor.prototype, attribute, {
                enumerable: true,
                get: function() {
...
module.exports.cachedValue = function cachedValue(args) {
    let BaseCollectionClass = args.collectionClass
        ? args.collectionClass.prototype instanceof Map
            ? AMap
            : ASet
        : null;
...
/**
 * Returns given string after converting first letter to upper case.
 * @param   {string} string - String to convert first letter to upper case.
 * @returns {string} string - String converted first letter to upper case.
 */
module.exports.ucFirst = function ucFirst(string) {

...
Table.prototype.addForeignKey = function (args) {
    ...
    if (typeof referencesSchema !== 'object') { throw new Error("'" + args.foreignSchemaName + '\' schema cannot be found.'); }
    referencesTable  = referencesSchema.table(args.foreignTableName);
    if (typeof referencesTable !== 'object') { throw new Error("'" + args.foreignTableName + '\' table cannot be found.'); }

    isNewFK = typeof this.foreignKeyConstraint(args.constraintName) !== 'object';       // Is fkContraint to be added is new one.
    ...
    if (typeof column !== 'object') { throw new Error("'" + this.schema().name() + '.' + this.name() + '.' + args.columnName + '\' column cannot be found.'); }
    referencesColumn = fkConstraint.referencesTable().column(args.foreignColumnName);
    if (typeof referencesColumn !== 'object') { throw new Error("'" + args.foreignSchemaName + '.' + args.foreignTableName + '.' + args.foreignColumnName + '\' column (referenced by foreign key) cannot be found.'); }

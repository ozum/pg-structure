...
DB.prototype.schema = helper.getObject('schemas', null, { before: function (name) {
    ...
    var requestedSchemas = lodash.isArray(internal(this).attributes.options.schemas) ? internal(this).attributes.options.schemas : [internal(this).attributes.options.schemas];
    if (!this.schemaExist(name) && requestedSchemas.indexOf(name) === -1) { throw new Error('Schema "' + name + '" does not exist. This schema is also not in the options: [' + requestedSchemas + ']. Perhaps you forget to add it to options.'); }
}});

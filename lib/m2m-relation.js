...
class M2MRelation {
    ...
    generateName(strategy) {
        ...
        return strategy
            ? namingStrategy[strategy].call(this)
            : getNameFromConstraintName.call(this) ||
                  getNameFromDescriptionData.call(this) ||
...
function getNameComplex() {
    let inflectionMethod =
        this.targetTable.name[0].toUpperCase() === this.targetTable.name[0] ? "camelize" : "underscore";
    let name =
        this.joinTable.name + helper.ucFirst(helper.fkToRelationName(this.targetConstraint.columns.array[0].name)); // join table name + fk name
...
function getNameFromConstraintName() {
    let inflectionMethod =
        this.targetTable.name[0].toUpperCase() === this.targetTable.name[0] ? "camelize" : "underscore";
    let string = this.sourceConstraint.name.includes(",")
        ? this.sourceConstraint.name.split(",")[2].replace(/\s/, "", "g")
        : undefined;
    if (string === undefined) return undefined;
    return inflection.transform(string + helper.ucFirst(this.targetTable.name), ["pluralize", inflectionMethod]);
}

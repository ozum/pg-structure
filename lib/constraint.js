...
function getForeignConstraint() {
    return internal.get(this).registry.getCollection('referentialConstraint').find({'leftJoinConstraintFullCatalogName': this.fullCatalogName, 'rightJoinConstraintFullCatalogName': null})[0];
}
...
/**
 * Returns array of {@link Column columns} belonging to this {@link Constraint constraint}.
 * @returns {Array.<Column>}        - Array of columns belonging to this constraint. Null if no
 * @private
 */
function getColumns() {
    let _ = internal.get(this);
    return _.registry.getCollection('constraintColumn').chain()

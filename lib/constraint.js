...
let parameters  = ['name', 'type', 'onUpdate', 'onDelete', 'matchOption', 'table', 'parent', 'description'];
let attributes  = ['name', 'type', 'onUpdate', 'onDelete', 'matchOption', ['parent', 'referencedTable'], ['description', 'comment'],
    'columns', 'referencedColumnsBy', ['table', 'child'], ['descriptionData', 'commentData']];
...
class Constraint {
     ...
     * @param   {string} args.description   - Description of the constraint.
    ...
    constructor(args) {
        ...
        _.descriptionData       = helper.extractJSON(args.description);
        _.columns               = new AMap();
        _.referencedColumnsBy   = new AMap();
    }
    ...
    serialize() {
        ...
        result.columns              = Array.from(_.columns).map(i => i[1].name);
        result.referencedColumnsBy  = Array.from(_.referencedColumnsBy).map(i => [i[0], i[1].fullName]);

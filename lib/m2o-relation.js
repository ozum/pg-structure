...
function getNameSimple() {
    ...
    return inflection.transform(this.targetTable.name, ['singularize', inflectionMethod]);
}
...
function getNameComplex() {
    let inflectionMethod = this.targetTable.name[0].toUpperCase() === this.targetTable.name[0] ? 'camelize' : 'underscore';
    let name   = helper.fkToRelationName(this.constraint.columns.array[0].name);        // First foreign key column name. Transform: company_id -> company or company -> related_company
    return inflection.transform(name, ['singularize', inflectionMethod]);
}

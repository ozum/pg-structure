 ...
 * @param {Object.<TableName,Object.<table>>} Structure object of the database.
 */
...
var Structure = function(host, database, username, password, options) {
    ...
    var result = {};
    var defaultTableObject = { columns: {}, columnsOrdered: [], primaryKeys: [] }
    var Structure = this;
    ...
    function processColumns(dbResult, next) {
        ...
        for (var i = 0; i < dbResult.rows.length; i++) {
            ...
            if (!result[column.tableName]) {
                result[column.tableName] = _.cloneDeep( defaultTableObject );
        ...
        for (var i = 0; i < dbResult.rows.length; i++) {
            ...
            if (index.unique) {
                ...
                index.columnNames.substr( 1, index.columnNames.length - 2).split(',').forEach(function(columnName) {
                    table.columns[columnName].unique = index.name;
                });
            }
        }
        next();
    ...
    function processRelations(next) {
        for ( var tableName in result ) {
            ...
            for (var firstConstraint in table.foreignKeys) {                                            // Process all foreign keys
                var firstTable = result[table.foreignKeys[firstConstraint][0].referencesTable];
                ...
                for (var secondConstraint in table.foreignKeys) {                                       // Process all foreign keys in inner loop for through relationships
                    if (firstConstraint == secondConstraint) {                                          // Skip same
                        continue
                    }
                    var secondTable = result[table.foreignKeys[secondConstraint][0].referencesTable];
 ...
 * @property {...column} columns - An object which contains column names as keys, column details as values.

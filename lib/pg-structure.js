...
function getClient(pgOptions) {
    return new Promise((resolve, reject) => {
        if (typeof pgOptions.query === 'function') { return resolve(pgOptions); }   // If it has a query method, assume it is a pg client.
...
function executeSqlFile(file, client, schemas) {
    return new Promise((resolve, reject) => {
        fs.readFile(path.join(sqlDir, file), (err, sql) => {
            if (err) { return reject(err); }
            return resolve(client.query(sql.toString(), [schemas]));
        });
    });
}
        ...
        query.on('row', (row) => {
            ...
            let constraint = table.constraints.get(row.constraintName)
                || table.constraints.set(row.constraintName, new Constraint({ schemaName: row.constraintSchema,
                    name: row.constraintName, type: row.constraintType, onUpdate: row.updateRule,
...
module.exports.save = function save(file, db) {
    if (!(db && file)) { return Promise.reject(new Error('Path and DB object is required.')); }
    return new Promise((resolve, reject) => {
    ...
    for (let serializedSchema of serializedDB.schemas) {
        ...
        for (let serializedTable of serializedSchema.tables) {
            ...
            for (let serializedConstraint of serializedTable.constraints) {
                if (serializedConstraint.parent) { serializedConstraint.parent = db.get(serializedConstraint.parent); }
